...
<template>
  <header class="bg-black px-4 py-2 flex justify-between items-center border-b-1" :class="[
      'transition ease-linear duration-700 border-gray-900 bg-gray-50 flex border-b w-full fixed z-50',
      visible ? 'transition ease-linear duration-700' : 'hidden transition ease-linear duration-700'
    ]">
      <div class="font-bold">On mange quoi</div>
      <UDropdown :items="items" :popper="{ placement: 'bottom-start' }">
          <UButton color="white" icon="i-heroicons-user" trailing-icon="i-heroicons-chevron-down-20-solid" />
      </UDropdown>
  </header>
